...
$stripeApiKey = $config['stripe_sk_key'];

            // Set your secret key (from Stripe Dashboard)
            \Stripe\Stripe::setApiKey($stripeApiKey); // Replace with your own Secret Key
                    ...
                    /*if (in_array($fileType, $allowedFileTypes)) { */
                        if (file_put_contents($fileName, $imageData)) {
                            try {
                                ...
                                $sql = "INSERT INTO articles (`title`, `image`, `author`, `date`, `content`, `tags`, `price`, `plan`, `paymentid`,`ReceiptURL`, `user_id`,`duration`,`category`) 
                                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)";
                                $stmt = $conn->prepare($sql);
                                $stmt->bind_param('sssssssssssss', $title, $cardimage1,$author, $date, $editorContent, $tags, $price, $plan, $paymentid, $ReceiptURL, $username,$duration,$category);
