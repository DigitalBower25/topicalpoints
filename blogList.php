...
<body>
    ...
    while($row = $result->fetch_assoc()) {?>
                            <label class="flex items-center space-x-2">
                                <input type="checkbox" class="w-4 h-4 category-checkbox" value="<?= $row["categories"];?>" />
    ...
    while($row = $result->fetch_assoc()) {?>
            ...
            <div class="md:col-span-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3  lg:grid-cols-1 gap-8" id="article-container">
            </div>
...
<script>
    ...
    <a class="page-link block py-2 px-4 text-center ${page === data.total_pages
        ...
        <span class="sr-only">Next</span>
                        ...
                        const articlesImg = `<div class="flex no-data flex-col items-center justify-center p-8 bg-gray-100 rounded-lg shadow-md mb-4">
...
`;                      container.removeClass('lg:grid-cols-3');
    ...
    $(".removeCategory").on("click", function () {
       alert('removed'); $(".category-checkbox").on("change");fetchArticles(1, "", "");
    });
    });    
