...
require('../mailer.php');
$id=$_POST['id'];
$sql = "SELECT * FROM articles WHERE id = '" . $id . "' and plan='Business'";
            $result = $conn->query($sql);
            $row = $result->fetch_assoc();

            if($row['id']===$id){
            //if($row['updatecount']!='2'){    
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                /*if (isset($_FILES['imagePreview']) && $_FILES['imagePreview']['error'] === UPLOAD_ERR_OK) {
                    ...
                    $fileName = basename($_FILES['imagePreview']['name']);
    ...
    if (file_put_contents($fileName, $imageData)) {
                    ...
                    $cardimagePreview = 'fetch/'.$fileName;
                    $title = $_POST['title'];
                    $author = $_POST['author'];
                    $tags = $_POST['tags'];
                    $status='Pending';
                    ...
                    //if (in_array($fileType, $allowedFileTypes)) {
                        //if (move_uploaded_file($fileTmpPath, $destPath)) {
                                ...
                                // print_r(array($title, $cardimagePreview, $cont_image1, $cont_image2, $author, $date, $editorContent, $tags, $price, $plan, $paymentid, $username));
                                $sql = "UPDATE articles SET `title`=?, `image`=?,`content`=?,`category`=?,`approvestatus`=?,`updatecount`=? WHERE id=?";
                                $stmt = $conn->prepare($sql);
                                $stmt->bind_param('sssssss', $title, $cardimagePreview, $editorContent,$catogory,$status,$updatecount,$id);
 ...
 <body>
  <div class="email-container">
   ...
   <div class="content">
    ...
    <h1>'.$title.'</h1>
        <p>Thank you for your choose Us ! Your article has been Updated and is currently under review. Our team will evaluate the article for compliance with our guidelines. You can expect a confirmation of approval within 24 to 48 hours.</p>
